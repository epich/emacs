      ...
      (while (setq next (pop list))     ;Exit inner loop at undo boundary.
        (pcase next
          ...
          (`(,(and marker (pred markerp)) . ,(and offset (pred integerp)))
           ...
           ;; release.  (Bug#16818)
           (undo-primitive-elt next))
          (_ (undo-primitive-elt next))))
      (setq arg (1- arg)))
    ;; Make sure an apply entry produces at least one undo entry,
    ;; so the test in `undo' for continuing an undo series
    ;; will work right.
    ;; TODO: Consider this in light of rework
    (if (and did-apply
...
(defun undo-primitive-elt (elt orig-undo-list)
  "Undo the ELT, which is one of the valid forms documented in
the Undo section of the Elisp manual. ORIG-UNDO-LIST is the cons
of buffer-undo-list which is undone and is inserted into
undo-redo-table. ELT can be eq to (car ORIG-UNDO-LIST), but need
not be if it has adjusted positions."
  (pcase elt
    ;; Handle an integer by setting point to that value.
    ((pred integerp) (goto-char elt))
    ...
    (`(nil . ,(or `(,prop ,val ,beg . ,end) pcase--dontcare))
     ...
     (puthash buffer-undo-list orig-undo-list undo-redo-table))
    ...
    (_ (error "Unrecognized entry in undo list %S" elt))))
